   ...
   public static function searchLawyers($req = null)
{
    if ($req != null) {
        ...
        if ($req->lawyer_category) {
            $lawyers = $lawyers->whereHas('lawyer_categories', function ($q) use ($req) {
                $q->where('slug', $req->lawyer_category);
            });
        }
        ...
        // Filter: Experience
        if ($req->experience) {
            $lawyers = $lawyers->where('experience', '>=', $req->experience);
        }

        // Filter: Rating (average from reviews)
        if ($req->rating) {
            $lawyers = $lawyers->whereHas('reviews', function ($q) use ($req) {
                $q->select('lawyer_id')
                  ->groupBy('lawyer_id')
                  ->havingRaw('AVG(rating) >= ?', [$req->rating]);
            });
        }

        // Filter: Fee / Consultation Fee (appointment_schedules.fee)
        if ($req->fee) {
            $lawyers = $lawyers->whereHas('appointmentSchedules', function ($q) use ($req) {
                $q->where('fee', '<=', $req->fee);
            });
        }

        // Filter: Language (lawyer_language -> all_languages.name)
        if ($req->language) {
            $lawyers = $lawyers->whereHas('languages', function ($q) use ($req) {
                $q->where('name', 'LIKE', "%{$req->language}%");
            });
        }

        // Filter: Years of Practice (lawyer_experiences.from <= X years ago)
        if ($req->years_of_practice) {
            $yearsAgo = now()->subYears($req->years_of_practice)->format('Y-m-d');
            $lawyers = $lawyers->whereHas('experiences', function ($q) use ($yearsAgo) {
                $q->whereDate('from', '<=', $yearsAgo);
            });
        }

        // Search by column or global
        if ($req->column && $req->search) {
            $lawyers = $lawyers->whereLike($req->column, $req->search);
        } elseif ($req->search) {
            $lawyers = $lawyers->whereLike(['first_name', 'last_name', 'description'], $req->search);
        }
        ...
        // Sorting
        if ($req->sort_field && $req->sort_type) {
            $lawyers = $lawyers->orderBy($req->sort_field, $req->sort_type);
        } else {
            $lawyers = $lawyers->orderBy('id', 'desc');
        }
    ...
    // Default response if $req is null
    $lawyers = Lawyer::withAll()->orderBy('id', 'desc');
    if ($req->all) {
        $lawyers = $lawyers->get();
    } else {
    ...
    // {
    //     if ($req != null) {
    //         $events =  Event::withAll()->active()->pastEvents();
    //         if ($req->month) {
    //             $months = [
...
public static function searchEvents($req = null)
{
    if ($req != null) {
        // ✅ Removed: ->pastEvents()
        $events = Event::withAll()->active(); // fetch all active events regardless of date
        ...
        // ⚠️ 'address' column doesn't exist — change this to correct columns if needed
        if ($req->address) {
            $events = $events->where(function ($q) use ($req) {
                $q->where('address_line_1', 'like', '%' . $req->address . '%')
                  ->orWhere('address_line_2', 'like', '%' . $req->address . '%');
            });
        }

        if ($req->column && $req->search) {
            $events = $events->where($req->column, 'like', '%' . $req->search . '%');
        } else if ($req->search) {
            $events = $events->where(function ($q) use ($req) {
    ...
    // ✅ Removed: ->upcoming() — show all events
    $events = Event::withAll()->active()->orderBy('id', 'desc')->paginate(10);
    $events = EventsResource::collection($events)->response()->getData(true);
    return $events;
}
